...
fn import_process_template(
    ...
    let template = UserProcessTemplate {
        ...
        display_name: Some(template_display_name(source.file_name().and_then(|value| value.to_str()).unwrap_or(""))),
        match_keywords: Some(infer_default_match_keywords(source.file_name().and_then(|value| value.to_str()).unwrap_or(""))),
...
#[tauri::command]
fn update_user_process_template(
    app: tauri::AppHandle,
    template: UserProcessTemplate,
...
#[tauri::command]
fn delete_user_process_template(app: tauri::AppHandle, path: String) -> Result<(), String> {
    let dir = process_template_dir(&app)?;
    let target = validate_user_template_path(&dir, &path)?;
    let target_path = path_to_string(target.clone());

    let mut templates = sync_user_process_templates(&dir)?;
    templates.retain(|item| item.user_template_path != target_path && item.user_template_path != path);
    write_user_process_templates(&dir, &templates)?;
...
fn normalize_user_template(mut template: UserProcessTemplate, path: &Path) -> Result<UserProcessTemplate, String> {
    let file_name = path
        .file_name()
        .and_then(|value| value.to_str())
        .ok_or_else(|| "无法识别模板文件名".to_string())?
...
fn infer_default_match_keywords(file_name: &str) -> Vec<String> {
    ...
    if let Some(index) = value.char_indices().find(|(_, ch)| is_cjk(*ch)).map(|(index, _)| index) {
